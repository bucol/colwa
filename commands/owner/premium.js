...
module.exports = {
    ...
    type: "owner",
	example: "\n\nContoh: %prefix%command add @tag/62812xxx 30d\nExample: %prefix%command del @tag/62812xxx",
    start: async (killua, m, { ar, args, prefix, command }) => {
		if (args.length < 2) return m.reply(`Contoh: ${prefix + command} add @tag/62812xxx 30d\nExample: ${prefix + command} del @tag/62812xxx`)
        if (ar[0] === 'add') {
			if (m.mentions.length !== 0) {
				for (let i = 0; i < m.mentions.length; i++) {
					user.addPremiumUser(m.mentions[0], args[2], _user);
					m.reply(`*「 PREMIUM DITAMBAHKAN 」*\n\n*ID :* ${args[1]}\n*Expired :* ${ms(toMs(args[2])).days} hari ${ms(toMs(args[2])).hours} jam ${ms(toMs(args[2])).minutes} menit`)
				}
			} else {
