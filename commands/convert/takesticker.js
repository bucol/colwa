module.exports = {
    ...
    start: async(killua, m, { command, prefix, args, quoted, mime }) => {
        if (!quoted) return  m.reply(`Reply stiker yang mau dicolong dengan command ${prefix + command}`)
        if (/image|video|sticker/.test(mime)) {
            ...
            return m.reply(`Reply stiker yang mau dicolong dengan command ${prefix + command}`, m.from, { quoted: m })
        }
    },
}
